...
def equivalent_from(a, b, ia, ib, seen=frozenset()):
    ...
    for i in range(l):
        ...
        if not equivalent_from(
            x, y, indices[x][a], indices[y][b], seen=s
        ):
            return False
    return True
...
def equivalent(a, b):
    ...
    l = len(graph[a])
    return all(
        equivalent_from(a, b, ia, ib)
        for ia, ib in it.product(range(l), repeat=2)
    )
...
def solve():
    cls = sorted(
        (sorted(cl) for cl in classes() if len(cl) > 1),
        key=lambda cl: cl[0],
    )
    if not cls:
        print("none")
